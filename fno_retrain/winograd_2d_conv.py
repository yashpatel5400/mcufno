# Source: https://github.com/xuexinghua/Operation-level-FI/blob/main/layer/winograd_layers.py
...
def untile(output, F, tiledShape, numChunks):
    output = output.reshape(
        output.shape[0], output.shape[1], tiledShape[2], tiledShape[3], F, F)
...
def pad_for_tiling(input, padding, F):
    ...
    if (number_tiling_positions).is_integer():
        ...
        Pad_tiling = torch.nn.ZeroPad2d(
            (to_pad_even, to_pad_even, to_pad_even, to_pad_even))
        print("Pad for tiling is {} for input {} and config F{}".format(
            Pad_tiling.padding, input.shape, F))
    return Pad_tiling
...
def win_conv2d(in_feature, kernel, padding, win_outtile, bits, stride, bias=None):
    ...
    img = F.pad(input=in_feature, pad=(padding, padding,
                padding, padding), mode='constant', value=0)
    ...
    array_AT, array_BT, array_G, Tarray_AT, Tarray_BT, Tarray_G = transfor(
        win_outtile)

    tiledShape, numChunks, input_ = tile(img, win_outtile, keh)
    input_ = input_.cuda()
    weight = kernel.unsqueeze(2)
    weight_winograd = torch.matmul(
        torch.matmul(array_G, weight), Tarray_G).cuda()
    input_winograd = torch.matmul(
        torch.matmul(array_BT, input_), Tarray_BT).cuda()
    input_winograd = input_winograd.unsqueeze(
        1).expand(-1, weight_winograd.shape[0], -1, -1, -1, -1)
    ...
    output = output.contiguous().view(
        output.shape[0], output.shape[1], output.shape[2]*output.shape[3]).transpose(1, 2)
    if bias is None:
